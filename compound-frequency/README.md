...
| `component_freq.fst` | 複合語の部品ごとの頻度表 |
...
追跡しない中間物は `.gitignore` に列挙してあります。`counts*/` の回数と見出しと記事名の TSV、`variants/` の成果物の変形、`sudachidict/` の解析辞書の配布物、`corpus/` の取得した技術文書とその平文がこれにあたり、どれも手順を通せば取り直せます。大きさは、複合語の回数だけで 857 MB、語と分野の組の回数で 1.6 GB あります。
...
| `query` | 成果物を引き、語のバケットと各部品の頻度を表示する |
...
| コマンド | 入力 | 時間 | 最大 RSS |
|---|---|---:|---:|
| `count --domains --dump` | 全記事 1,484,267 件 | 28 分 11 秒 | 1,242,448 kB |
| `article-titles` | 全記事 1,484,267 件 | 3 分 31 秒 | 262,048 kB |
| `dictionary-headwords` | 解析辞書 full | 3.7 秒 | 458,688 kB |
| `count --domains --text-dir` | 技術文書 13,848 文書 | 2.8 秒 | 206,928 kB |
| `count --domains --corpus-dir` | 日本語コーパス 14,540 文書 | 25.9 秒 | 351,280 kB |
| `build` | 3 つの回数と見出しと記事名の TSV | 12.7 秒 | 3,114,704 kB |
...
読んだ全記事は 1,484,267 件、数えた本文は 9,285,571,411 バイトです。解析に失敗して飛ばした記事はありません。複合語のキーは 42,730,803 件、部品は 1,505,390 件です。語と分野の組は 74,244,912 件で、分野ごとの記事の数は `design.md` の「分野の区分」にあります。

技術文書は、配布用が 16 ソースの 13,848 文書、平文 54,872,563 バイトで、キーは 155,454 件、部品は 12,022 件です。評価用は 14 ソースの 13,564 文書、53,725,896 バイトで、キーは 152,146 件、部品は 11,762 件です。どちらも解析に失敗して飛ばした文書はありません。文書単位で数えるので、1 つのキーの回数は文書の数を超えません。

日本語コーパスは、配布用が 20 ソースの 14,540 文書、平文 535,840,821 バイトで、キーは 670,181 件、部品は 47,675 件です。評価用は 18 ソースの 14,400 文書、533,972,365 バイトで、キーは 658,368 件、部品は 46,937 件です。どちらも文語体の法令 284 件を外してあります。
...
|---|---:|---:|---:|---:|---:|---:|
| 配布用 | 11,183,905 | 25,165,910 | 629,274 | 5,855,317 | 7,891,906 | 17,760,342 |
| 評価用 | 11,182,650 | 25,165,910 | 629,221 | 5,857,575 | 7,891,749 | 17,760,342 |

2 つの差は、キー 1,255 件と部品 53 件、それに語と分野の組 157 件だけです。外した 4 ソースが持つ語の大半が、Wikipedia の本文か記事名か解析辞書の見出しにもあるためです。`freq_filter.bin` のバイト数が同じなのは、フィルタの配列がキーの数を段階で丸めた大きさを取るためです。
