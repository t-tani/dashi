...
corpus-tool の `query` で引くと、語ごとにバケットと部品の値が並びます。部品の値は `<部品>:<出現数>:<前・外来語>:<前・漢語>:<後ろ・外来語>:<後ろ・漢語>` の形で、5 つの欄は `component_freq.fst` の値を復号したものです。分割単位 A の部品の後ろに、分割単位 C で 1 語になり A で 2 形態素以上に割れる語が同じ形で並びます。

```
cargo run --release -- query --artifacts-dir .. 公開鍵暗号 幽霊参照
公開鍵暗号	7	公開:262144:1327:2896:608:2435 鍵:7193:38:215:75:279 暗号:17109:181:279:77:512
幽霊参照	未登録	幽霊:8192:27:139:47:394 参照:42494:469:1116:90:197
```
...
複合語の部品ごとの頻度表です。形式は fst 0.4.7 の `Map` で、キーが部品の文字列、値が 5 つの欄を詰めた `u64` です。フィルタと違って、キーの一覧の取り出し、接頭辞での検索、値そのものの取り出しができます。

部品は、複合語の区間に現れた形態素の表層形です。正規化形は持たないため、引くときも表層形で引きます。区間の外に現れた名詞は数えません。見出しには、この分割単位 A の部品と、分割単位 C で 1 語になり分割単位 A で 2 形態素以上に割れる語(`母集団`・`読み取り専用`)の両方があります。後者の足し方は `design.md` の「部品の相手の種類数」にあります。

読み込むのは corpus-tool だけです。`query` が各部品の 5 つの欄を並べ、`judge` が部品の出現数の比 `min(f(X), f(Y)) / (f(XY) + 1)` で造語の候補の理由を「部品」と「未登録」に分けます。構成の分析(`judge --concatenation composition`)も、1 形態素の単位の頻度をこの表から取ります。

### 値の欄

値は上位から次の欄に区切ります。符号化と復号の関数は akunuki の `aku_freq` の `ConstituentEntry` が持ち、corpus-tool は書くときに `encode` を、読むときに `decode` を呼びます。

| bit | 欄 | 幅 | 中身 |
|---|---|---:|---|
| 63〜61 | 形式のバージョン | 3 | この形式では 1 |
| 60〜43 | 予約 | 18 | 0 で埋める |
| 42〜35 | 後ろ・漢語 | 8 | 部品の後ろに漢語が付いた複合語の種類数 |
| 34〜27 | 後ろ・外来語 | 8 | 部品の後ろに外来語が付いた複合語の種類数 |
| 26〜19 | 前・漢語 | 8 | 部品の前に漢語が付いた複合語の種類数 |
...
どの欄も、小さい値をそのままの整数で持ち、大きい値を桁の刻みで持ちます。刻みは 2 倍ごとに決まった段数を割り当て、段は符号化前の値を超えない側に切り捨てます。出現数は 1,023 以下をそのままの整数にし、種類数は 127 以下をそのままの整数にします。

```
encode_frequency(n) = n                                    if n <= 1023
                    = 1024 + floor(16 * log2(n / 1024))    otherwise      // 最大 2047
encode_count(n)     = n                                    if n <= 127
                    = 128 + floor(8 * log2(n / 128))       otherwise      // 最大 255

value = 1 << 61
      | encode_count(後ろ・漢語)   << 35
      | encode_count(後ろ・外来語) << 27
      | encode_count(前・漢語)     << 19
      | encode_count(前・外来語)   << 11
      | encode_frequency(出現数)
```

復号は、そのままの整数の範囲では元の値を、桁の刻みの範囲では区間の下端を返します。

```
decode_frequency(c) = c                                    if c <= 1023
                    = floor(1024 * 2 ^ ((c - 1024) / 16))  otherwise
decode_count(c)     = c                                    if c <= 127
                    = floor(128 * 2 ^ ((c - 128) / 8))     otherwise
```

出現数 1,024 は刻みの最初の段の下端なので、そのまま戻ります。1,500 は段 8 に入り、1,448 に戻ります。どの欄も閾値との比較にしか使わないので、この精度で足ります。`judge` の部品の比の閾値 1000 はそのままの整数の範囲にあるため、丸めは判定を変えません。

### 旧形式

形式のバージョンの欄が 0 の値は旧形式で、値の全体が出現数です。`dashi-2026.09.3` までの成果物がこの形です。旧形式の出現数は 2^61 未満なので、この欄が 0 になることと旧形式であることは一致します。読む側は旧形式の値を出現数だけとして読み、種類数は 4 つとも 0 にします。
...
Manifest {
    ...
    component_freq_format_version: u64      // component_freq.fst の値の形式のバージョン(1)
